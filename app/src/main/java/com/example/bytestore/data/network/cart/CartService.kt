...
class CartService(private val context: Context) {
    ...
    suspend fun addItem(productId: Long, qty: Int, unitPriceCents: Long): ListCartItemsModel? =
        withContext(Dispatchers.IO) {
            ...
            Log.d(
                TAG,
                "ADD Item - Product ID: $productId, Qty: $qty, Price: $unitPriceCents centavos"
            )
            ...
            if (current == null || !current.isSuccessful || current.body() == null) {
                ...
                if (createResp != null) {
                    Log.d(
                        TAG,
                        "POST Response - Success: ${createResp.isSuccessful}, Code: ${createResp.code()}"
                    )
                ...
                val itemsPayload =
                    existing.map { CreateCartItemDto(it.productId, it.quantity, it.unitPrice) }
                ...
                if (cartId <= 0) {
                    ...
                    runCatching { api.createCart(body) }.getOrNull()?.takeIf { it.isSuccessful }
                        ?.body()?.toModel()
                } else {
                    ...
                    if (putResp != null) {
                        Log.d(
                            TAG,
                            "PUT Response - Success: ${putResp.isSuccessful}, Code: ${putResp.code()}"
                        )
    ...
    suspend fun updateQty(productId: Long, qty: Int): ListCartItemsModel? =
        withContext(Dispatchers.IO) {
            val user =
                runCatching { session.getCurrentUser() }.getOrNull() ?: return@withContext null
            val current =
                runCatching { api.getByUser(user.id) }.getOrNull() ?: return@withContext null
            val cartDto = current.body() ?: return@withContext null
            val existing =
                cartDto.items.map { if (it.productId == productId) it.copy(quantity = qty) else it }
            val itemsPayload =
                existing.map { CreateCartItemDto(it.productId, it.quantity, it.unitPrice) }
    ...
    suspend fun removeItem(productId: Long): ListCartItemsModel? = withContext(Dispatchers.IO) {
        ...
        val itemsPayload =
            remaining.map { CreateCartItemDto(it.productId, it.quantity, it.unitPrice) }
        val cartId = cartDto.id ?: return@withContext null
        runCatching { api.replaceCart(cartId, ReplaceCartRequest(itemsPayload)) }.getOrNull()
            ?.takeIf { it.isSuccessful }?.body()?.toModel()
    }
     ...
     */
    suspend fun createCartFromPayload(items: List<CreateCartItemDto>): ListCartItemsModel? =
        withContext(Dispatchers.IO) {
            val user =
                runCatching { session.getCurrentUser() }.getOrNull() ?: return@withContext null
            val body = CreateCartRequest(userId = user.id, items = items)
            runCatching { api.createCart(body) }.getOrNull()?.takeIf { it.isSuccessful }?.body()
                ?.toModel()
        }
}
